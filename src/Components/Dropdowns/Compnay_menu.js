...
export default function Menu() {
  const [query, setQuery] = useSearchParams({ sort: "Date Added", page: "1" });
  ...
  const [hideKeywords, setHideKeywords] = useState([]);
  ...
  useEffect(() => {
    ...
    if (query.get("location")) {
      setLocation(
        getOriginalArray(query.get("location").split(","), mainLocationOptions),
        mainLocationOptions
      );
    } else {
      setLocation([]);
